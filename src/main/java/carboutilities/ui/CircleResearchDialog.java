package carboutilities.ui;
// The code from AQUARION by TwCash was used as a basis.
...
public class CircleResearchDialog extends BaseDialog {

    public static boolean debugShowRequirements = false;

    public final float nodeSize = Scl.scl(mindustry.Vars.mobile ? 80f : 60f);
    ...
    private static final float RING_SPACING = Scl.scl(300f);

    public CircleResearchDialog() {
        super("");
    ...
    public void rebuildItems() {
        items = new ItemSeq() {
            ObjectMap<Sector, ItemSeq> cache = new ObjectMap<>();
            {
                ...
                if (rootPlanets.isEmpty()) rootPlanets = Seq.with(Planets.serpulo);
    ...
    int getSubtreeSize(TechTreeNode node) {
        ...
        int sum = 0;
        for (TechTreeNode child : node.children) sum += getSubtreeSize(child);
        return sum;
    }

    void layoutRadial(TechTreeNode node, float startAngle, float endAngle, int depth) {
        ...
        for (int i = 0; i < node.children.length; i++) {
            ...
            float ratio = (float) weights[i] / totalWeight;
            float arc = (endAngle - startAngle) * ratio;
            float angle = cursor + arc / 2f;
            float rad = angle * Mathf.degreesToRadians;
            float r = RING_SPACING * depth;

            child.x = Mathf.cos(rad) * r;
            child.y = Mathf.sin(rad) * r;

            layoutRadial(child, cursor, cursor + arc, depth + 1);
            cursor += arc;
        }
    }
    ...
    int getMaxDepth(TechTreeNode node, int depth) {
        int max = depth;
        for (TechTreeNode child : node.children) max = Math.max(max, getMaxDepth(child, depth + 1));
        return max;
    }
    ...
    public class View extends Group {
            ...
            for (TechTreeNode node : nodes) {
                ...
                button.update(() -> {
                    ...
                    button.getStyle().up = !locked(node.node) ? Tex.buttonOver
                            ...
                            : Tex.button;
                    ((TextureRegionDrawable) button.getStyle().imageUp).setRegion(
                            node.selectable ? node.node.content.uiIcon : Icon.lock.getRegion());
            ...
            if (mobile) {
                addListener(new InputListener() {
                    ...
                    public boolean touchDown(InputEvent event, float x, float y, int pointer, KeyCode button) {
                        ...
                        if (e == View.this) { hoverNode = null; rebuild(); }
                        return false;
                    }
                });
            }
        ...
        void spend(TechNode node) {
            ...
            for (int i = 0; i < node.requirements.length; i++) {
                ...
                if (used > 0) { shine[i] = true; usedShine[req.item.id] = true; }
                if (completed.amount < req.amount) complete = false;
            }

            if (complete) unlock(node);
            node.save();
            Core.scene.act();
            rebuild(shine);
        ...
        void unlock(TechNode node) {
            ...
            while (parent != null) { parent.content.unlock(); parent = parent.parent; }
            checkNodes(root);
            hoverNode = null;
            treeLayout();
            rebuild();
        ...
        void rebuild(@Nullable boolean[] shine) {
            ...
            infoTable.addListener(new InputListener() {
                ...
                public void exit(InputEvent event, float x, float y, int pointer, Element fromActor) {
                    Element e = Core.scene.hit(
                            Core.input.mouseX(pointer == -1 ? 0 : pointer),
                            Core.input.mouseY(pointer == -1 ? 0 : pointer), true);
                    if (hoverNode == button && !(e != null && (e == infoTable || e.isDescendantOf(infoTable)
                            || e == hoverNode || e.isDescendantOf(hoverNode)))
            ...
            infoTable.table(b -> {
                            ...
                            desc.table(t -> {
                                ...
                                if (selectable) {
                                    if (Structs.contains(node.finishedRequirements, s -> s.amount > 0)) {
                                        ...
                                        arc.scene.ui.Label label = t.add(Core.bundle.format("research.progress",
                                                Math.min((int)(used / sum * 100), 99))).left().get();
                                        if (shiny) { label.setColor(Pal.accent); label.actions(Actions.color(Color.lightGray, 0.75f, Interp.fade)); }
                                        else label.setColor(Color.lightGray);
                                        t.row();
                                    }
                                    for (int i = 0; i < node.requirements.length; i++) {
                                        ItemStack req = node.requirements[i];
                                        ...
                                        t.table(list -> {
                                            ...
                                            arc.scene.ui.Label lbl = list.label(() -> " " +
                                                    UI.formatAmount(Math.min(items.get(req.item), reqAmount)) + " / " +
                                                    UI.formatAmount(reqAmount)).get();
                                            Color target = items.has(req.item) ? Color.lightGray : Color.scarlet;
                                            if (shiny) { lbl.setColor(Pal.accent); lbl.actions(Actions.color(target, 0.75f, Interp.fade)); }
                                            else lbl.setColor(target);
                                        }).fillX().left();
                                        t.row();
                                    }
                                } else if (node.objectives.size > 0) {
                                    t.table(r -> {
                                        r.add("@complete").colspan(2).left(); r.row();
                ...
                // Android: кнопка "Исследовать" прямо в popup
                if (mobile && locked(node) && !net.client()) {
                    b.row();
                    b.button("@research", Icon.ok, new TextButtonStyle() {{
                        disabled = Tex.button; font = Fonts.def; fontColor = Color.white;
                        disabledFontColor = Color.gray; up = buttonOver; over = buttonDown;
        ...
        // Рисует правильный шестиугольник с заданным радиусом
        private void drawHexagon(float cx, float cy, float radius) {
            float[] vx = new float[6];
            float[] vy = new float[6];
            for (int i = 0; i < 6; i++) {
                float a = (float)(i * 60) * Mathf.degreesToRadians;
                vx[i] = cx + Mathf.cos(a) * radius;
                vy[i] = cy + Mathf.sin(a) * radius;
            }
            for (int i = 0; i < 6; i++) {
                Lines.line(vx[i], vy[i], vx[(i + 1) % 6], vy[(i + 1) % 6]);
            }
        }

        // Рисует волнообразную линию между двумя точками
        private void drawWavyLine(float x1, float y1, float x2, float y2, float amplitude, int segments) {
            ...
            for (int i = 1; i <= segments; i++) {
                ...
                float wave = Mathf.sin(t * Mathf.PI * 4f) * amplitude;
                float qx = x1 + dx * t + nx * wave;
                float qy = y1 + dy * t + ny * wave;
                Lines.line(px, py, qx, qy);
                px = qx;
                py = qy;
            }
        }
        ...
        public void drawChildren() {
            ...
            for (int i = 1; i <= maxDepth; i++) {
                float radius = RING_SPACING * i;

                Draw.z(0f);
                Draw.color(Pal.darkerGray);
                Lines.stroke(Scl.scl(2.5f));
                drawHexagon(offsetX, offsetY, radius);

                for (int e = 1; e <= 2; e++) {
                    Lines.stroke(Scl.scl(0.8f) / e);
                    Draw.color(0.15f, 0.15f, 0.22f, 0.4f / e);
                    drawHexagon(offsetX, offsetY, radius + RING_SPACING * 0.12f * e);
                }
            }
            ...
            for (TechTreeNode node : nodes) {
                ...
                for (TechTreeNode child : node.children) {
                    ...
                    Draw.z(lock ? 1f : 2f);
                    Lines.stroke(Scl.scl(2.5f), lock ? Pal.gray : Pal.accent);
                    Draw.alpha(parentAlpha * (lock ? 0.5f : 1f));

                    float x1 = node.x + offsetX,  y1 = node.y + offsetY;
                    float x2 = child.x + offsetX, y2 = child.y + offsetY;
                    float dist = Mathf.dst(x1, y1, x2, y2);
                    int segs = Math.max(8, (int)(dist / 15f));
                    float amp = lock ? 3f : 6f;

                    drawWavyLine(x1, y1, x2, y2, amp, segs);
                }
            }
